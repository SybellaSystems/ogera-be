...
export const PermissionChecker = (route: string, action: string) => {
  return async (req: any, _res: Response, next: NextFunction) => {
    try {
      ...
      // Role.permission_json is already JSON because your model uses Sequelize.JSON
      const permissions: any[] = role.permission_json || [];
      ...
      for (const perm of permissions) {
        ...
        if (isMatch) {
          ...
          return next(
            new CustomError("Forbidden: You cannot perform this action", 403)
          );
        }
      }

      return next(
        new CustomError("Forbidden: No permission rule for this route", 403)
      );
    } catch (error) {
