...
export const uploadAcademicDoc = async (
    req: any,
    res: Response,
): Promise<void> => {
    try {
...
export const getAcademicVerificationDocument = async (
    req: any,
    res: Response,
): Promise<void> => {
    try {
        ...
        console.log(
            `[VIEW_DOC] Request for doc ID: ${id}, User: ${userId}, Role: ${role}`,
        );

        const verification = await getAcademicVerificationByIdService(id);
        console.log(
            `[VIEW_DOC] Verification found. Storage type: ${verification.storage_type}, Path: ${verification.document_path}`,
        );
        ...
        console.log(
            `[VIEW_DOC] Attempting to fetch local file: ${document_path}`,
        );
        const fileBuffer = getLocalFile(document_path);
        if (!fileBuffer) {
            console.error(
                `[VIEW_DOC] File buffer is null for path: ${document_path}`,
            );
            response.errorResponse(
                res,
        ...
        console.log(
            `[VIEW_DOC] Sending file with content type: ${contentType}, size: ${fileBuffer.length} bytes`,
        );
...
export const reuploadAcademicDoc = async (
    req: any,
    res: Response,
): Promise<void> => {
    try {
...
export const reviewAcademicDoc = async (
    req: any,
    res: Response,
): Promise<void> => {
    try {
        ...
        const result = await reviewAcademicDocService(
            id,
...
export const getAcademicVerificationById = async (
    req: any,
    res: Response,
): Promise<void> => {
    try {
...
export const getAcademicVerificationByUserId = async (
    req: any,
    res: Response,
): Promise<void> => {
    try {
...
export const getAllAcademicVerifications = async (
    req: any,
    res: Response,
): Promise<void> => {
    try {
        ...
        if (roleName === 'student' || roleType === 'student') {
            try {
                const myVerifications =
                    await getAcademicVerificationByUserIdService(userId);
                const verifications = Array.isArray(myVerifications)
                    ? myVerifications
                    : myVerifications
                    ? [myVerifications]
                    : [];
                ...
                const filtered = status
                    ? verifications.filter((v: any) => v.status === status)
                    : verifications;
        ...
        const result = await getAllAcademicVerificationsService({
            page,
            limit,
            status,
        });
...
export const getPendingAcademicVerifications = async (
    req: any,
    res: Response,
): Promise<void> => {
    try {
        ...
        const result = await getPendingAcademicVerificationsService({
            page,
            limit,
        });
