...
const repo = {
    ...
    findByUserId: async (
        user_id: string,
    ): Promise<AcademicVerification | null> => {
        return await DB.AcademicVerifications.findOne({
            where: { user_id },
    ...
    update: async (
        id: string,
        updates: Partial<AcademicVerification>,
    ): Promise<void> => {
    ...
    findAll: async ({
        ...
        limit,
        status,
    }: PaginationQuery & { status?: AcademicVerificationStatus }): Promise<{
        rows: AcademicVerification[];
        count: number;
    }> => {
        const where: any = {};
        if (status) {
