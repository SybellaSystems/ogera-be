...
import {
    ...
    ReviewAcademicVerificationData,
} from '@/interfaces/academicVerification.interfaces';
...
export const uploadAcademicDocService = async (
    ...
    const { path: document_path, storageType } = await saveFile(
        file,
        'academic-proofs',
    );
    ...
    if (verifyDocAdminRole) {
        ...
        if (verifyDocAdminUsers && verifyDocAdminUsers.length > 0) {
            ...
            console.log(
                `Verification assigned to verifyDocAdmin user: ${assignedAdminId} (${minPendingCount} pending verifications)`,
            );
        }
    }
    ...
    if (!assignedAdminId) {
        console.warn(
            'No verifyDocAdmin user found. Verification created without assignment.',
        );
    }
...
export const reuploadAcademicDocService = async (
    ...
    if (!existing) {
        throw new CustomError(
            'Academic verification not found',
    ...
    if (!updated) {
        throw new CustomError(
            'Failed to retrieve updated verification',
...
export const getAcademicVerificationByUserIdService = async (
    user_id: string,
) => {
