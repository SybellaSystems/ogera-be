...
import {
  JWT_ACCESS_TOKEN_SECRET as JWT_SECRET,
} from "@/config";
...
export const registerUser = async (data: any) => {
  ...
  if (exists) throw new CustomError("Email already exists", StatusCodes.CONFLICT);
...
export const loginUser = async (body: any) => {
  ...
  if (!role) throw new CustomError("User role not found", StatusCodes.INTERNAL_SERVER_ERROR);
  ...
  if (user.two_fa_enabled) {
    if (!body.otp) {
      throw new CustomError("2FA OTP Required", StatusCodes.PARTIAL_CONTENT);
    }

    const otpValid = await verifyOTP(user.user_id, body.otp);
    if (!otpValid) throw new CustomError("Invalid 2FA OTP", StatusCodes.BAD_REQUEST);
  }
