...
export const getStudentDashboard = async (
    ...
    try {
        ...
        const metrics: StudentDashboardResponse =
            await dashboardService.getStudentDashboard(userId, periodDays);
        ...
        logger.error(
            '[Dashboard Controller] Error in getStudentDashboard:',
            error,
        );
        res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({
            success: false,
...
export const getEmployerDashboard = async (
    ...
    try {
        ...
        const periodDays = Number(req.query.periodDays) || 30;
        const metrics: EmployerDashboardResponse =
            await dashboardService.getEmployerDashboard(userId, periodDays);
        ...
        logger.error(
            '[Dashboard Controller] Error in getEmployerDashboard:',
            error,
        );
        res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({
            success: false,
...
export const getRecentActivities = async (
    ...
    try {
        ...
        logger.error(
            '[Dashboard Controller] Error in getRecentActivities:',
            error,
        );
        res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({
            success: false,
