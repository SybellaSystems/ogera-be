...
const repo = {
    ...
    getTotalEarnings: async (): Promise<number> => {
        try {
            ...
            const jobs = (await DB.Jobs.findAll({
                ...
                amount_paid_to_student?: number | string | null;
            }>;
            ...
            for (const job of jobs) {
                ...
                if (job.funding_status === 'Paid') {
                    const paidAmount =
                        ...
                        job.amount_paid_to_student !== undefined
                            ? Number(job.amount_paid_to_student)
                            : Math.round(totalForJob * 0.9);
                    totalPaidToStudents += Number.isNaN(paidAmount)
                        ? 0
                        : paidAmount;
                }
            }
            ...
            const walletBalance = Math.max(
                0,
                Math.round(totalReceived - totalPaidToStudents),
            );
            logger.info(
                `[Dashboard] Total earnings (wallet balance): ${walletBalance}`,
            );
    ...
    getRecentActivities: async (limit = 5) => {
        try {
            ...
            logger.error(
                '[Dashboard] Error fetching recent activities:',
                error,
            );
            return [];
        }
     ...
     */
    getStudentApplicationsCountBetween: async (
        studentId: string,
        start: Date,
        end: Date,
    ): Promise<number> => {
        try {
            ...
            if (count && count > 0) {
                logger.info(
                    `[Dashboard] Student ${studentId} applications between ${start.toISOString()} and ${end.toISOString()}: ${count} (from JobApplications)`,
                );
                return Number(count ?? 0);
            }
            ...
            try {
                ...
                logger.info(
                    `[Dashboard] Student ${studentId} applications between ${start.toISOString()} and ${end.toISOString()}: ${altCount} (from ActivityLogs fallback)`,
                );
            ...
            logger.error(
                '[Dashboard] Error counting student applications:',
                error,
            );
            return 0;
        }
    ...
    getStudentAcceptedCount: async (studentId: string): Promise<number> => {
        try {
            ...
            logger.error(
                '[Dashboard] Error counting accepted applications:',
                error,
            );
            return 0;
        }
     ...
     */
    getStudentEarningsBetween: async (
        studentId: string,
        start: Date,
        end: Date,
        ...
        try {
            ...
            const total = paidApplications.reduce((sum, application) => {
                const jobId = String(
                    application?.job?.job_id ?? application?.job_id ?? '',
                );
                if (!jobId || seenJobIds.has(jobId)) return sum;
                seenJobIds.add(jobId);

                const paid = Number(
                    application?.job?.amount_paid_to_student ?? 0,
                );
                return sum + (Number.isNaN(paid) ? 0 : paid);
            }, 0);

            logger.info(
                `[Dashboard] Student ${studentId} MoMo earnings between ${start.toISOString()} and ${end.toISOString()}: ${total}`,
            );
    ...
    getStudentTotalEarnings: async (
        ...
        try {
            ...
            logger.info(
                `[Dashboard] Student ${studentId} total MoMo earnings: ${total}`,
            );
            return { total, currency: null };
        } catch (error) {
            logger.error(
                '[Dashboard] Error summing student total earnings:',
                error,
            );
            return { total: 0, currency: null };
        }
     ...
     */
    getEmployerJobsPostedBetween: async (
        employerId: string,
        start: Date,
        end: Date,
    ): Promise<number> => {
        try {
            ...
            logger.error(
                '[Dashboard] Error counting employer jobs posted:',
                error,
            );
            return 0;
        }
     ...
     */
    getEmployerApplicationsReceivedBetween: async (
        employerId: string,
        start: Date,
        end: Date,
    ): Promise<number> => {
        try {
            ...
            logger.error(
                '[Dashboard] Error counting employer applications:',
                error,
            );
            return 0;
        }
     ...
     */
    getEmployerActiveHiresBetween: async (
        employerId: string,
        start: Date,
        end: Date,
    ): Promise<number> => {
        try {
            ...
            logger.error(
                '[Dashboard] Error counting employer active hires:',
                error,
            );
            return 0;
        }
     ...
     */
    getEmployerApplicationStatusBreakdown: async (
        employerId: string,
    ): Promise<{
        pending: number;
        shortlisted: number;
        ...
        try {
            const rows: Array<{ status: string; count: string }> =
                (await DB.JobApplications.findAll({
                    include: [
                        {
                            model: DB.Jobs,
                            as: 'job',
                    ...
                    attributes: [
                        ...
                        [
                            DB.sequelize.fn(
                                'COUNT',
                                DB.sequelize.col(
                                    'JobApplicationModel.application_id',
                                ),
                            ),
                            'count',
                        ],
            ...
            for (const r of rows) {
                ...
                else if (s === 'shortlisted' || s === 'reviewed')
                    breakdown.shortlisted += n;
            ...
            logger.error(
                '[Dashboard] Error computing employer application breakdown:',
                error,
            );
            return {
                pending: 0,
    ...
    > => {
        try {
            ...
            logger.error(
                '[Dashboard] Error fetching recent applicants:',
                error,
            );
            return [];
        }
    ...
    getEmployerSpentBetween: async (
        ...
        try {
            ...
            logger.error(
                '[Dashboard] Error summing employer spent amount:',
                error,
            );
            return 0;
        }
     ...
     */
    getStudentJobsCompletedBetween: async (
        studentId: string,
        start: Date,
        end: Date,
    ): Promise<number> => {
        try {
            ...
            logger.info(
                `[Dashboard] Student ${studentId} jobs completed between ${start.toISOString()} and ${end.toISOString()}: ${count}`,
            );
            return Number(count ?? 0);
        } catch (error) {
    ...
    getStudentWeeklyActivity: async (
        studentId: string,
    ): Promise<
        Array<{ day: string; applications: number; completed: number }>
    > => {
        try {
            ...
            for (const d of days) {
                ...
                const completed = await repo.getStudentJobsCompletedBetween(
                    studentId,
                    d.start,
                    d.end,
                );
            ...
            logger.error(
                '[Dashboard] Error building student weekly activity:',
                error,
            );
            return [];
        }
    ...
    getEmployerWeeklyActivity: async (
        employerId: string,
    ): Promise<Array<{ day: string; applications: number; hires: number }>> => {
        try {
            const days = repo.buildLast7Days();
            const out: Array<{
                day: string;
            ...
            for (const d of days) {
                const applications =
                    await repo.getEmployerApplicationsReceivedBetween(
                        employerId,
                        d.start,
                        d.end,
                    );
                const hires = await repo.getEmployerActiveHiresBetween(
                    employerId,
                    d.start,
                    d.end,
                );
            ...
            logger.error(
                '[Dashboard] Error building employer weekly activity:',
                error,
            );
            return [];
        }
    ...
    getAdminWeeklyGrowth: async (): Promise<
        Array<{ day: string; students: number; employers: number }>
    > => {
        try {
            const { Op } = require('sequelize');
            const days = repo.buildLast7Days();
            ...
            for (const d of days) {
                const [students, employers] = await Promise.all([
                    DB.Users.count({
                        where: {
                            role_type: 'student',
                            created_at: { [Op.gte]: d.start, [Op.lt]: d.end },
                        },
                    }),
                    DB.Users.count({
                        where: {
                ...
                out.push({
                    ...
                    employers: Number(employers ?? 0),
                });
            }
            return out;
        } catch (error) {
            logger.error(
                '[Dashboard] Error building admin weekly growth:',
                error,
            );
            return [];
        }
    ...
    getStudentApplicationStatusBreakdown: async (
        studentId: string,
    ): Promise<{
        pending: number;
        shortlisted: number;
        ...
        try {
                ...
                (await DB.JobApplications.findAll({
                    ...
                    attributes: [
                        ...
                        [
                            DB.sequelize.fn(
                                'COUNT',
                                DB.sequelize.col('application_id'),
                            ),
                            'count',
                        ],
                    ],
            ...
            logger.error(
                '[Dashboard] Error computing application status breakdown:',
                error,
            );
            return {
                pending: 0,
        ...
        try {
            ...
            const jobCompletionRate =
                acceptedApplications > 0
                    ? Math.round(
                          (completedApplications / acceptedApplications) * 100,
                      )
                    : 0;
     ...
     */
    getStudentInterviewsBetween: async (
        studentId: string,
        start: Date,
        end: Date,
    ): Promise<number> => {
        try {
            ...
            logger.info(
                `[Dashboard] Student ${studentId} interviews between ${start.toISOString()} and ${end.toISOString()}: ${count}`,
            );
            return Number(count ?? 0);
        } catch (error) {
            logger.error('[Dashboard] Error counting interviews:', error);
    ...
    getNewStudentsThisWeek: async (): Promise<number> => {
        try {
            ...
            logger.info(`[Dashboard] New students this week: ${count}`);
    ...
    getNewJobsPostedThisWeek: async (): Promise<number> => {
        try {
            ...
            logger.info(`[Dashboard] New jobs posted this week: ${count}`);
    ...
    getPendingAcademicVerificationsCount: async (): Promise<number> => {
        try {
            ...
            logger.info(`[Dashboard] Pending academic verifications: ${count}`);
    ...
    getResolvedDisputesCount: async (): Promise<number> => {
        try {
            ...
            logger.info(`[Dashboard] Resolved disputes: ${count}`);
    ...
    getNewApplicantsThisWeek: async (): Promise<number> => {
        try {
            ...
            logger.info(`[Dashboard] New applicants this week: ${count}`);
    ...
    getPositionsFilledThisMonth: async (): Promise<number> => {
        try {
            ...
            logger.info(`[Dashboard] Positions filled this month: ${count}`);
