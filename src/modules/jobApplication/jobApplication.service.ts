...
export const getEmployerApplicationsService = async (
    ...
    if (roleType === 'employer') {
        const result = await repo.findAllApplicationsForEmployer(
            user_id,
            statusFilter,
            currentPage,
            pageLimit,
        ...
        if (status === 'Accepted') {
            try {
                ...
                if (
                    ...
                    const conversation =
                        await messagesRepo.createOrGetConversationRepo(
                            updatedApplication.job.employer_id,
                            updatedApplication.student_id,
                            updatedApplication.job_id,
                        );
                    console.log(
                        `✓ Conversation ${conversation.conversation_id} created/retrieved for approved application ${application_id}`,
                    );
                } else {
                    console.warn(
                        `⚠️ Missing required data for conversation creation: employer_id=${updatedApplication?.job?.employer_id}, student_id=${updatedApplication?.student_id}, job_id=${updatedApplication?.job_id}`,
                    );
                }
            } catch (error) {
                console.error(
                    'Failed to create conversation for approved application:',
                    error,
                );
