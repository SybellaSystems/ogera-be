    ...
    return Promise.all(
        notifications.map(async (notification: any) => {
            const notificationData = notification.toJSON
                ...
                : notification;
            if (
                (notificationData.type === 'job_application' ||
                ...
                try {
                    ...
                    if (application) {
                        notificationData.application = application.toJSON
                            ? application.toJSON()
                            : application;
                    }
                } catch (error) {
                    console.error(
                        'Error fetching application details for notification:',
                        error,
                    );
                }
            }
...
const repo = {
     ...
     */
    findAllNotifications: async (options?: {
        is_read?: boolean;
        limit?: number;
    }) => {
        const where: any = {};
