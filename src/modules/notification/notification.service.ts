...
export const sendAdminNotificationService = async (params: {
    ...
    const normalizedSenderRole = (params.sender_role || '')
        .toLowerCase()
        .trim();
    ...
    const targetRoles =
        targetMode === 'role'
            ? Array.from(
                  new Set(
                      (params.target_roles || []).map(r => r.toLowerCase()),
                  ),
              ).filter(r => r === 'student' || r === 'employer')
            : [];
    const targetUserIds =
        targetMode === 'specific'
            ? Array.from(
                  new Set((params.target_user_ids || []).filter(Boolean)),
              )
            : [];
    ...
    const recipients = await repo.findUsersForNotificationTargets(
        targetMode,
        targetRoles,
        targetUserIds,
    );
    if (!recipients.length) {
        throw new CustomError(
            'No recipients found for selected target',
    ...
    const createdNotifications = await repo.createNotificationsBulk(
        ...
        await Promise.all(
            recipients.map(async (recipient: any) => {
                ...
                try {
                    ...
                    console.error(
                        `Failed sending notification email to ${recipient.email}:`,
                        error,
                    );
                }
            }),
...
export const getNotificationsService = async (
    ...
    if (isSuperAdmin) {
        return await repo.findAllNotificationsForSuperAdminInbox(
            user_id,
            options,
        );
    }
...
export const createMessageNotification = async (params: {
    ...
    const recentEmailNotification =
        await repo.findRecentMessageEmailNotification(
            params.receiver_id,
            params.conversation_id,
            recentWindowStart,
        );
    ...
    const chatUrl = `${frontendBase.replace(
        /\/$/,
        '',
    )}/dashboard/messages?conversationId=${params.conversation_id}`;
