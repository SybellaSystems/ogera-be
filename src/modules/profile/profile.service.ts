...
async function refreshTrustScoreAfterProfileChange(
    user_id: string,
): Promise<void> {
    try {
        ...
        logger.warn(
            `TrustScore refresh after profile change failed for ${user_id}: ${
                err?.message || err
            }`,
        );
    }
...
export const addBulkSkillsService = async (
    user_id: string,
...
export const updateSkillService = async (
    ...
    if (skill.user_id !== user_id) {
        throw new CustomError(
            'Unauthorized to update this skill',
...
// ====================== EMPLOYMENT ======================
export const addEmploymentService = async (
    user_id: string,
    ...
    if (data.is_current) {
        ...
        for (const emp of existingEmployments) {
            if (emp.is_current) {
                await repo.updateEmployment(emp.employment_id, {
                    is_current: false,
                });
            }
        }
...
export const updateEmploymentService = async (
    user_id: string,
    ...
    if (employment.user_id !== user_id) {
        throw new CustomError(
            'Unauthorized to update this employment',
            StatusCodes.FORBIDDEN,
        );
    }
...
export const deleteEmploymentService = async (
    user_id: string,
    employment_id: string,
) => {
    const employment = await repo.findEmploymentById(employment_id);
    ...
    if (employment.user_id !== user_id) {
        throw new CustomError(
            'Unauthorized to delete this employment',
            StatusCodes.FORBIDDEN,
        );
    }
...
export const updateEducationService = async (
    ...
    if (education.user_id !== user_id) {
        throw new CustomError(
            'Unauthorized to update this education',
            StatusCodes.FORBIDDEN,
        );
    }
...
export const deleteEducationService = async (
    ...
    if (education.user_id !== user_id) {
        throw new CustomError(
            'Unauthorized to delete this education',
            StatusCodes.FORBIDDEN,
        );
    }
...
export const updateProjectService = async (
    ...
    if (project.user_id !== user_id) {
        throw new CustomError(
            'Unauthorized to update this project',
            StatusCodes.FORBIDDEN,
        );
    }
...
// ====================== ACCOMPLISHMENTS ======================
export const addAccomplishmentService = async (
    user_id: string,
...
export const getAccomplishmentsService = async (
    user_id: string,
    accomplishment_type?: string,
) => {
    const accomplishments = await repo.findAccomplishmentsByUserId(
        user_id,
        accomplishment_type,
    );
...
export const updateAccomplishmentService = async (
    user_id: string,
    ...
    if (accomplishment.user_id !== user_id) {
        throw new CustomError(
            'Unauthorized to update this accomplishment',
            StatusCodes.FORBIDDEN,
        );
    }
...
export const deleteAccomplishmentService = async (
    user_id: string,
    accomplishment_id: string,
) => {
    const accomplishment = await repo.findAccomplishmentById(accomplishment_id);
    ...
    if (accomplishment.user_id !== user_id) {
        throw new CustomError(
            'Unauthorized to delete this accomplishment',
            StatusCodes.FORBIDDEN,
        );
    }
...
export const updateExtendedProfileService = async (
    user_id: string,
    data: UpdateExtendedProfileRequest,
) => {
    // DEBUG LOG: Log data being sent to repository
    console.log(
        '[SERVICE] updateExtendedProfileService - Data received:',
        JSON.stringify(data, null, 2),
    );

    const extendedProfile = await repo.createOrUpdateExtendedProfile(
        user_id,
        data,
    );
    ...
    console.log(
        '[SERVICE] updateExtendedProfileService - Result from DB:',
        extendedProfile,
    );
...
export const updateCompanyInfoService = async (
    ...
    // DEBUG LOG: Log data being sent to repository
    console.log(
        '[SERVICE] updateCompanyInfoService - Data received:',
        JSON.stringify(data, null, 2),
    );
    ...
    try {
        ...
        // DEBUG LOG: Log result from repository
        console.log(
            '[SERVICE] updateCompanyInfoService - Result from DB:',
            companyProfile,
        );
        ...
        logger.error(
            `Failed to save company information for ${user_id}: ${
                error?.message || error
            }`,
        );
...
// ====================== ONLINE PRESENCE ======================
export const updateOnlinePresenceService = async (
    user_id: string,
    data: UpdateOnlinePresenceRequest,
) => {
    // DEBUG LOG: Log data being sent to repository
    console.log(
        '[SERVICE] updateOnlinePresenceService - Data received:',
        JSON.stringify(data, null, 2),
    );
    ...
    try {
        ...
        // DEBUG LOG: Log result from repository
        console.log(
            '[SERVICE] updateOnlinePresenceService - Result from DB:',
            result,
        );
        ...
        logger.error(
            `Failed to save online presence for ${user_id}: ${
                error?.message || error
            }`,
        );
        if (error instanceof CustomError) {
        ...
        throw new CustomError(
            'Failed to save online presence',
            StatusCodes.INTERNAL_SERVER_ERROR,
        );
    }
};
...
export const getProfileCompletionService = async (user_id: string) => {
    ...
    if (user.full_name) completedFields.push('full_name');
    else missingFields.push('full_name');
    if (user.email_verified) completedFields.push('email_verified');
    else missingFields.push('email_verified');
    if (user.phone_verified) completedFields.push('phone_verified');
    else missingFields.push('phone_verified');
    if ((user as any).profile_image_url) completedFields.push('profile_image');
    else missingFields.push('profile_image');

    const bio = fullProfile.extendedProfile?.profile_summary || '';
    if (bio.length >= 20) completedFields.push('bio');
    else missingFields.push('bio');

    if (role === 'student') {
        if (user.resume_url) completedFields.push('resume');
        else missingFields.push('resume');
        if (fullProfile.skills && fullProfile.skills.length >= 3)
            completedFields.push('skills');
        else missingFields.push('skills');
        if (fullProfile.educations && fullProfile.educations.length >= 1)
            completedFields.push('education');
        else missingFields.push('education');
        if (fullProfile.employments && fullProfile.employments.length >= 1)
            completedFields.push('employment');
        else missingFields.push('employment');
        if (fullProfile.projects && fullProfile.projects.length >= 1)
            completedFields.push('projects');
        else missingFields.push('projects');
        if (user.preferred_location) completedFields.push('preferred_location');
        else missingFields.push('preferred_location');
    } else if (role === 'employer') {
        if (user.business_registration_id)
            completedFields.push('business_registration');
        else missingFields.push('business_registration');
        if (bio.length >= 50) {
            if (!completedFields.includes('bio'))
                completedFields.push('company_description');
        } else {
    ...
    > = {
        ...
        skills: {
            title: 'Skills',
            description: 'Add at least 3 key skills',
            icon: 'star',
        },
        education: {
            title: 'Education',
        ...
        preferred_location: {
            title: 'Location',
            description: 'Set your preferred location',
            icon: 'location',
        },
        business_registration: {
            title: 'Business Registration',
            description: 'Add your business registration ID',
            icon: 'building',
        },
        company_description: {
    ...
    return {
        ...
        profile_completed_at:
            missingFields.length === 0 ? new Date().toISOString() : null,
        missingFields,
        completedFields,
        badges: [],
        wizardSteps,
...
export const getOtherUserFullProfileService = async (
