...
const ensureApprovedAssignment = async (job_id: string, student_id: string) => {
    const approvedApplication =
        await repo.findApprovedApplicationForJobAndStudent(job_id, student_id);
...
const buildSummary = (
    ...
    const disputedTaskCount = tasks.filter(
...
export const getEmployerTaskOverviewService = async (user_id: string) => {
    ...
    const overview = await Promise.all(
        jobs.map(async (job: any) => {
            ...
            const plainJob =
                typeof job.get === 'function' ? job.get({ plain: true }) : job;
...
export const getMyAssignedTasksService = async (user_id: string) => {
    ...
    if (roleType !== 'student') {
        throw new CustomError(
            'Only students can view assigned tasks',
            StatusCodes.FORBIDDEN,
        );
    }
...
export const getJobTaskManagementService = async (
    job_id: string,
    user_id: string,
) => {
    ...
    try {
        ...
        if (studentEmail) {
            await sendTaskAssignedEmail(studentEmail, {
                ...
                jobTitle,
                taskTitle:
                    String(plain.title || payload.title || '').trim() ||
                    'New task',
...
export const updateTaskStatusService = async (
    ...
    if (nextStatus === 'COMPLETED' && task.assigned_student_id) {
        checkAndAwardPioneerBadge(task.assigned_student_id).catch(() => {
            // Intentionally ignore badge-awarding errors so task completion is not affected.
        });
    }
