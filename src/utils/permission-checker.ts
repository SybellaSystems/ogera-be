import { RoutePermission } from '@/interfaces/role.interfaces';
...
export class PermissionChecker {
     ...
     */
    static getAccessibleRoutes(permissions: RoutePermission[]): string[] {
        if (!permissions || !Array.isArray(permissions)) {
            return [];
        }
     ...
     * @returns string[] - Array of route names
     */
    static getRoutesWithPermission(
        permissions: RoutePermission[],
        action: PermissionAction,
    ): string[] {
        if (!permissions || !Array.isArray(permissions)) {
    ...
    static hasAllPermissions(
        permissions: RoutePermission[],
        checks: Array<{ route: string; action: PermissionAction }>,
    ): boolean {
        return checks.every(check =>
            this.hasPermission(permissions, check.route, check.action),
        );
    }
    ...
    static parsePermissions(
        permissionJson: string | RoutePermission[],
    ): RoutePermission[] {
     ...
     */
    static getPermissionSummary(permissions: RoutePermission[]): {
        totalRoutes: number;
        fullAccessRoutes: number;
        readOnlyRoutes: number;
        noAccessRoutes: number;
        partialAccessRoutes: number;
    } {
        if (!permissions || !Array.isArray(permissions)) {
            return {
                totalRoutes: 0,
        ...
        const fullAccessRoutes = permissions.filter(p =>
            this.hasFullAccess(permissions, p.route),
        ).length;
        const readOnlyRoutes = permissions.filter(p =>
            this.hasReadOnlyAccess(permissions, p.route),
        ).length;
        const noAccessRoutes = permissions.filter(
            p => !this.hasAnyPermission(permissions, p.route),
        ).length;
        const partialAccessRoutes =
            totalRoutes - fullAccessRoutes - readOnlyRoutes - noAccessRoutes;
