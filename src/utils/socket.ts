...
const getConversationRoom = (conversation_id: string) =>
...
const isUserOnline = (user_id: string) =>
    (userSocketIds.get(user_id)?.size || 0) > 0;

export const isUserActiveInConversation = (
    user_id: string,
    conversation_id: string,
) => userConversationActivity.get(user_id)?.has(conversation_id) || false;

export const getUserLastSeenAt = (user_id: string) =>
    userLastSeenAt.get(user_id) || null;

const broadcastConversationPresence = (
    conversation_id: string,
    user_id: string,
) => {
    if (!io) return;
    ...
    io.on('connection', (socket: Socket) => {
        ...
        socket.on('join_conversation', async (conversation_id: string) => {
            try {
                const { allowed, conversation } =
                    await verifyConversationAccess(conversation_id, user_id);
        ...
        socket.on(
            ...
            async (payload: { conversation_id?: string }) => {
                ...
                const { allowed } = await verifyConversationAccess(
                ...
                socket
                    .to(getConversationRoom(payload.conversation_id))
